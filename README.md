...
1. Follow the instructions at [Install Rust](https://www.rust-lang.org/tools/install) in the _Rust documentation_\.
...
To build on Windows, follow the instructions at [Set up your dev environment on Windows for Rust](https://learn.microsoft.com/en-us/windows/dev-environment/rust/setup) in the _Microsoft Windows documentation_\.
...
The following instructions show how to get a secret named _MyTest_ by using the example script `secrets-manager-agent-extension.sh` in [https://github\.com/aws/aws\-secretsmanager\-agent](https://github.com/aws/aws-secretsmanager-agent) to install the Secrets Manager Agent as a Lambda extension\.
...
The agent can be configured using a TOML file. Here are the available options:

// ... existing options ...

- `sso_profile`: (Optional) The AWS SSO profile name to use for credentials. If not specified, the default credential chain will be used.
...
ssrf_env_variables = ["AWS_TOKEN", "AWS_SESSION_TOKEN"]
path_prefix = "/v1/"
max_conn = 800
